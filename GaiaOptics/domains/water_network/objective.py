...
Minimal tank + pump model constraints, consistent with GaiaOptics core contracts:
- Return List[ConstraintResult]
- Margins: >= 0 satisfied, < 0 violated
- Use Severity.HARD for feasibility constraints

Starter constraints:
1) tank_level_min (HARD): tank_level_m3[t] >= tank_min_m3
2) tank_level_max (HARD): tank_level_m3[t] <= tank_max_m3
3) pump_power_cap (HARD): pump_power_kw[t] <= p_max_kw
"""
...
def evaluate_constraints_water_network(
    ...
    # pump_power_cap: margin = min_t(p_max - p[t])
