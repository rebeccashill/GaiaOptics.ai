# gaiaoptics/visualization/data_center_plots.py
from __future__ import annotations

"""
Data center plotting utilities (Phase 3 polish).

Goal:
- Produce 1 simple, credible plot:
    room temperature over time
- Read outputs/<scenario>/traces.csv and write outputs/<scenario>/plots/temperature_over_time.png

This module is defensive:
- Tries multiple common column names for time and temperature.
...
def plot_temperature_over_time(
    ...
    *,
    ...
    temp_col = _pick_column(header, _TEMP_CANDIDATES)

    t = _to_float_list(rows, time_col)
    temp = _to_float_list(rows, temp_col)

    plt.figure()
    plt.plot(t, temp)
    plt.xlabel(time_col)
    plt.ylabel(temp_col)
    plt.title(title or "Data center temperature over time")
    plt.tight_layout()
...
def plot_data_center_outputs_dir(outputs_dir: Path) -> Path:
    """
    Convenience helper:
      outputs/<scenario>/traces.csv -> outputs/<scenario>/plots/temperature_over_time.png
    """
    traces_csv = outputs_dir / "traces.csv"
    out_png = outputs_dir / "plots" / "temperature_over_time.png"
    return plot_temperature_over_time(
        traces_csv,
        out_png,
        title=f"Data center temperature over time — {outputs_dir.name}",
    )