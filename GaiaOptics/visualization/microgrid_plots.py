# gaiaoptics/domains/microgrid/microgrid_plots.py
from __future__ import annotations

"""
Microgrid plotting utilities (Phase 3 polish).

Goal:
- Produce 1 simple, credible plot:
    cost over time
- No dashboards, no styling drama—just a clean matplotlib PNG.

Expected inputs:
- traces.csv produced in outputs/<scenario_name>/traces.csv
- Must contain a time column (t or time) and a cost-ish column.

This module is intentionally defensive:
- It tries multiple common column names for time and cost.
- If it can’t find a suitable column, it raises a clear error.
"""
...
def plot_cost_over_time(
    ...
    *,
    ...
    cost_col = _pick_column(header, _COST_CANDIDATES)

    t = _to_float_list(rows, time_col)
    cost = _to_float_list(rows, cost_col)

    plt.figure()
    plt.plot(t, cost)
    plt.xlabel(time_col)
    plt.ylabel(cost_col)
    plt.title(title or "Microgrid cost over time")
    plt.tight_layout()
...
def plot_microgrid_outputs_dir(outputs_dir: Path) -> Path:
    """
    Convenience helper:
      outputs/<scenario>/traces.csv -> outputs/<scenario>/plots/cost_over_time.png
    """
    traces_csv = outputs_dir / "traces.csv"
    out_png = outputs_dir / "plots" / "cost_over_time.png"
    return plot_cost_over_time(traces_csv, out_png, title=f"Microgrid cost over time — {outputs_dir.name}")